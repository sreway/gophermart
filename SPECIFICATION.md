...
- адрес подключения к Kafka: переменная окружения ОС `KAFKA_BROKER_ADDRESS`
- тип подключения producer к Kafka: переменная окружения ОС `KAFKA_BROKER_NETWORK`
- имя используемого topic в Kafka: переменная окружения ОС `KAFKA_TOPIC`
- номер используемого partition в Kafka: переменная окружения ОС `KAFKA_TOPIC`
